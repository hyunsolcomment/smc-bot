import { Embed, EmbedBuilder, GuildMember, Message, Role, User, ChannelType, CategoryChannel } from "discord.js";
import { client } from "../bot";
import { PermissionResolvable } from "discord.js";

const TeamCommand = {
    execute: async (e: Message<boolean>, user: User, args: string[]) => {

        // 도움말 출력
        if(args.length === 0 || args?.[0] === "도움말") {
            e.reply({ embeds: [
                new EmbedBuilder().setColor("Green").setTitle("팀 도움말")
        ...
        const teamPermission: PermissionResolvable = ['ViewChannel','SendMessages','Connect'];
        ...
        switch(args[0]) {
            case "생성":
                ...
                // 팀 역할 생성
                let role: Role = await e.guild.roles.create({ name: `HiuTeam_${teamName}` });

                // 유저들에게 팀 역할 부여하기
                users.forEach(user => user.roles.add(role));

                // 카테고리 생성
                const category = await e.guild.channels.create({ name: teamName, type: ChannelType.GuildCategory });
                ...
                await category.children.create({ type: ChannelType.GuildText, name: "일반" });
                await category.children.create({ type: ChannelType.GuildVoice, name: "일반" });

                console.log("작업 끝!");
                
                // 작업 완료 출력하기
                e.reply({ embeds: [ new EmbedBuilder().setColor("Green").setTitle(`${teamName} 팀(을)를 생성했어요.`).setDescription(usersStr.length > 0 ? `${usersStr}(이)가 팀에 포함되었습니다!` : '아무도 팀에 포함되지 않았어요. \n`!팀 팀원추가 [팀 이름] [@유저들 언급]`으로 누군가를 팀에 포함시킬 수 있습니다!') ] });
                break;
            ...
            case "삭제":
                ...
                if(teamRole) {
                    ...
                    e.guild.channels.cache.filter(c => c.type === ChannelType.GuildCategory && !c.permissionsFor(e.guild.roles.everyone).has("ViewChannel") && c.permissionsFor(madeByRole).has(teamPermission) && c.permissionsFor(teamRole).has(teamPermission)).forEach((c:CategoryChannel) => {
                    ...
                    // 작업 완료 출력하기
                    e.reply({ embeds: [new EmbedBuilder().setColor("Green").setTitle(`${teamName} 팀(을)를 삭제했어요.`)] });
